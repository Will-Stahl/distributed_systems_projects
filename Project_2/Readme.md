...
<b>IMPORTANT NOTE:</b> There is a slight degree of randomness associated with our server tests for the all 3 consistencies which we were not able to find the root cause for. If the tests do fail, then please close all terminals and relaunch two fresh terminal windows in the `test` directory and run the above 2 commands (`java StartSystem sequential` and `java -cp ./../lib/junit-4.13.2.jar:./../lib/hamcrest-core-1.3.jar:. RunTestClass sequential`) again to run the tests (replacing `sequential` with `quorum` and `readyourwrites`. We weren't encountering any issues during manual testing, but our unit tests sometimes randomly fail. Again, this is a slightly random thing that happens and the tests are expected to run and execute correctly 95% of the time. Tests may also occasionally fail due to connection errors (`java.net.ConnectException: Connection refused`) If this happens, please run them again after restarting `StartSystem`.
